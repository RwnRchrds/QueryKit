...
namespace QueryKit.Extensions
{
    ...
    public static class ConnectionExtensions
    {
            ...
            IDbTransaction? transaction = null, int? commandTimeout = null)
        {
            ...
            {
                if (Config.SupportsInsertReturning)
                {
                    // RETURNING names the row that was just written. The alternative on PostgreSQL,
                    // LASTVAL(), reports the last value taken from any sequence in the session, so
                    // a trigger writing to another table would hand back that table's key instead.
